  ...
  <Section testId="about-credentials" tone="surface">
    ...
    <RevealList className="mt-12 grid gap-5 md:grid-cols-3">
      {credentials.map((credential) => (
        <RevealItem className="h-full" key={credential.abbreviation}>
          <div
            className="flex h-full flex-col rounded-2xl border border-hairline bg-surface-muted p-6"
            data-testid={`about-credential-${credential.abbreviation.toLowerCase()}`}
          >
            <span className="grid h-11 w-11 place-items-center rounded-xl bg-accent-soft text-accent">
              <GraduationCap aria-hidden="true" className="h-5 w-5" />
            </span>
            <h3 className="mt-5 text-base font-semibold text-slateink">{credential.title}</h3>
            <p className="mt-1 text-xs font-medium uppercase tracking-[0.12em] text-slateink-soft">
              {credential.issuer}
            </p>
            <p className="mt-3 text-sm leading-relaxed text-slateink-muted">{credential.detail}</p>
          </div>
        </RevealItem>
      ))}
    </RevealList>
